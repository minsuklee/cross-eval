...
> **v2.3 변경**: 교수 평가 미완료 시 종료 차단, 평가마감 전 경고 추가, 교수 평가 현황 표시.

```
교수 → "평가 종료" 버튼 클릭
  → 프론트엔드에서 사전 검증:
    1. 교수평가여부 = Y 인 과제: 모든 제출 학생에 대해 교수 평가가 완료되었는지 확인
       → 미완료 시: "교수 평가가 완료되지 않은 학생이 N명 있습니다. 모든 교수 평가를 완료한 후 종료해주세요." 알림 → 종료 차단
    2. 평가마감일시가 아직 지나지 않은 경우:
       → "평가 마감일시(YYYY-MM-DD HH:MM)가 아직 지나지 않았습니다. 그래도 종료하시겠습니까?" 확인 → 교수가 확인하면 종료 진행
  → 검증 통과 시 Apps Script가:
...
**규칙:**
- `교수평가여부 = Y`인 과제는 모든 제출 학생에 대한 교수 평가가 완료되어야만 평가를 종료할 수 있다
- 평가마감일시 이전에는 경고를 표시하되, 교수가 확인하면 종료할 수 있다 (강제 종료 가능)
- 교수 평가 현황(완료/전체)은 과제 관리 화면의 과제 카드에 표시한다
...
### 4.12 학생 결과 확인 (내 결과 탭)

> **v2.3 변경**: 학생 대시보드 "내 결과" 탭에서 과제 상태에 따라 적절한 안내를 표시. 확정 전에는 진행 상황 안내, 미제출 과제는 "미제출 종료" 표시.

```
학생 → GitHub Pages 로그인 → 대시보드 → "내 결과" 탭 클릭
  → 백엔드에서 get_my_results 호출 (모든 상태의 과제 반환)
  → 과제별 상태에 따른 표시:

    ▸ 상태 = "제출중":
      - "📋 [과제명]: 과제 제출 진행 중 (제출 마감: YYYY-MM-DD HH:MM)"
      - 미제출인 경우: "아직 제출하지 않았습니다."

    ▸ 상태 = "평가중":
      - "📝 [과제명]: 상호평가 진행 중 (평가 마감: YYYY-MM-DD HH:MM)"
      - "평가가 종료되고 교수가 결과를 확정하면 점수를 확인할 수 있습니다."
      - 미제출인 경우: "미제출 — 이 과제는 제출하지 않아 상호평가에서 제외되었습니다."

    ▸ 상태 = "평가완료":
      - "⏳ [과제명]: 교수 확정 대기 중"
      - "교수가 결과를 확정하면 평가 결과를 확인할 수 있습니다."
      - 미제출인 경우: "미제출 종료 — 이 과제는 제출하지 않아 평가 결과가 없습니다."
...
**규칙:**
- "내 결과" 탭은 확정 과제만 보여주는 것이 아니라, **모든 과제**의 현재 상태를 보여준다
- 미제출 상태로 상호평가가 시작되거나 확정된 경우 "미제출 종료"로 표시한다
- 모든 시간은 **KST (한국 표준시)**로 표시한다
...
> **v2.3 변경**: `get_my_results`가 확정 과제뿐 아니라 모든 과제의 상태와 제출 여부를 반환. 교수 평가 현황 정보를 `get_assignments_list`에 포함.

---

## 5.6 시간 표시 규칙 (v2.3 신규)

- **모든 시간은 KST (한국 표준시, UTC+9)**로 표시한다
- 백엔드에서 저장하는 시간은 ISO 8601 형식 (`new Date().toISOString()`)
- 프론트엔드에서 시간을 표시할 때 `toLocaleString('ko-KR', { timeZone: 'Asia/Seoul', ... })`으로 KST 변환
- 적용 대상: 제출 마감일시, 평가 마감일시, 제출 시각, 평가 완료 시각 등 모든 날짜/시간 표시
...
| 화면 | 파일 | 설명 |
|------|------|------|
| 과목 대시보드 | `admin/dashboard.html` | **과목 관리 탭** (과목 추가/선택/삭제/복원), 과제 목록, 상태별 관리, **상호평가 시작 버튼 (마감 전 가능)**, **제출 현황 + 교수 직접평가**, **교수 평가 현황 표시 (N/M명 완료)**, **평가종료 시 교수평가 미완료 차단**, **헤더에 현재 선택 과목명 표시** |
...
| 화면 | 파일 | 설명 |
|------|------|------|
| 대시보드 | `student/dashboard.html` | **전체 과목**의 과제 목록 (과목명 태그 표시), 상태별 액션, **내 결과 탭 (전체 과제 진행 상황 + 확정 결과)** |
| 과제 제출 | (대시보드 내 또는 모달) | **링크 입력 + 공유 안내 메시지 (v2.0 신규)** |
| 상호평가 | `student/evaluate.html` | **익명(학생1, 학생2) 표시**, 제출 링크 + 채점기준 + 점수/서술평 입력 |
| 내 결과 | (대시보드 내 탭) | **전체 과제 진행 상태 + 미제출 종료 표시 + 확정 시 평가 결과 (v2.3 변경)** |
