...
교수의 Apps Script 프로젝트에 바인딩된 마스터 스프레드시트. 글로벌 설정과 과목 목록을 관리한다.
...
> **v2.0 변경**: `course_name`, `semester` 는 마스터에서 제거. 각 과목 스프레드시트의 `_config`로 이동.
...
교수 대시보드 → "새 과목 만들기"
  ...
  → "만들기" 클릭
  → Apps Script가:
    1. Google Spreadsheet 생성: "CrossEval-2026-1-소프트웨어공학개론"
    2. _config, 학생_마스터, 과제_목록 시트 자동 생성
    3. 마스터 스프레드시트의 과목_목록에 레코드 추가
  → 과목 목록 화면으로 돌아감
```
...
학생 → 대시보드에서 "제출중" 과제 선택
  → 과제 내용 및 채점 기준 확인
  → 공유 링크 입력 (구글 드라이브, 구글 문서 등)
  → ⓘ "kookmin.ac.kr 도메인 계정을 가진 사람이 읽을 수 있도록 공유 설정을 해주세요" 안내
  → "제출하기" 클릭
  → 제출_A{n} 시트에 [학번, 제출링크, 제출시각] 저장
  → 이미 제출한 경우 기존 링크 표시, 수정 가능
  → 상호평가 시작 이후에는 수정 불가
```
...
학생 → GitHub Pages 로그인
  → 대시보드에서 "평가중" 과제 확인
  → 배정된 피평가자가 "학생1", "학생2" (익명) 버튼으로 표시
  → 버튼 클릭 시:
    - 피평가자의 제출 링크 표시 (새 탭에서 열림)
    - 평가 기준 표시
    - 점수 입력 (min~max 범위 내 정수)
    - 서술평 입력 (textarea, 최소 20자)
    - "평가 완료" → 평가_A{n} 시트에 기록
  → 완료된 학생은 "학생N(완료)" 표시
  → 완료 후 다시 클릭하면 이전 입력값 표시, 수정 가능
```

**제약:**
- 평가 마감일시 이후에는 제출 불가
- 한 번 제출한 평가는 마감 전까지 수정 가능
- 점수는 정수만 허용, 범위 검증
- **피평가자의 실제 이름/학번은 비공개** (학생1, 학생2 등으로만 표시)
...
| Action | Method | 파라미터 | 반환 | 설명 |
|--------|--------|---------|------|------|
| `create_assignment` | POST | adminPassword, courseId, name, desc, deadline, criteria, minScore, maxScore | 과제ID | 과제 생성 (**상태 즉시 `제출중`**) |
...
| 화면 | 파일 | 설명 |
|------|------|------|
| 과목 선택 | `admin/courses.html` | **과목 목록 카드**, "새 과목 만들기" 버튼 **(v2.0 신규)** |
| 과목 대시보드 | `admin/dashboard.html` | 과제 목록, 상태별 관리, **상호평가 시작 버튼 (마감 전 가능)** |
