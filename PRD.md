...
교수 → 대시보드 → "과목 관리" 탭 → 활성 과목의 "선택" 버튼 클릭
  → 세션에 선택된 과목 정보 저장 (courseId, courseName)
...
- 과목 선택 상태는 세션(sessionStorage)에 저장
...
| 화면 | 파일 | 설명 |
|------|------|------|
| 과목 대시보드 | `admin/dashboard.html` | **과목 관리 탭** (과목 추가/선택/삭제/복원), 과제 목록, 상태별 관리, **상호평가 시작 버튼 (마감 전 가능)**, **헤더에 현재 선택 과목명 표시** |
